...
public class Buff 
{
	...
	public ItemStack getBuffIcon()
	{
		...
		if (buffStats.meleeDamageAdd != 0)
			lore.add(CakeLibrary.recodeColorCodes("&7 * Melee Damage: +" + buffStats.meleeDamageAdd));
		if (buffStats.meleeDamageMultiplier != 0)
			lore.add(CakeLibrary.recodeColorCodes("&7 * Melee Damage: +" + CakeLibrary.convertMultiplierToAddedPercentage(buffStats.meleeDamageMultiplier) + "%"));
	...
	public void applyBuff(RPlayer rp)
	{
		...
		for (int i = 0; i < rp.buffs.size(); i++)
		{
			...
			{
				b.duration = duration;
				CakeCore.msgNoTag(p, "&e--- Buff &6[ " + buffStats.buffStatsName + "&6 ] &eapplied ---");
				return;
			}
		}
		rp.buffs.add(this);
	...
	{
		if (p != null)
			CakeCore.msgNoTag(p, "&7--- Buff &8[ " + buffStats.buffStatsName + "&8 ] &7expired ---");
	}
}
