...
public class NPCManager
{
	...
	public static final File skinDataFile = new File("plugins/CakeCore/SkinData.yml");
	public static final File skinDataFetchedFile = new File("plugins/CakeCore/SkinDataFetched.yml");
	public static final File npcFolder = new File("plugins/CakeCore/npcs");
	...
	public NPCManager(CakeCore instance)
	{
	...
	public void loadNPCs()
	{
		...
		for (File file: npcFolder.listFiles())
		{
			...
			{
				CakeCore.msgConsole("&4Error: Unable to read NPC File: " + file.getName());
			}
						...
						if(!interactDelay.contains(p))
						{
							...
							for(CustomNPC npc : npcs)
								if(npc.getId() == EntityID.get(packet) && p.getEyeLocation().distance(npc.getBukkitLocation()) < 7) //NPC INTERACT EVENT
								{
									...
									{
										rp.selectedNPC = npc;
										CakeCore.msg(p, "NPC Selected: " + npc.getName() + "&e (&6" + npc.databaseName + "&e)");
	...
	public void readSkinDataFile(File file, boolean fetched)
	{
		...
		for (String line: CakeLibrary.readFile(file))
		{
			try
			{
				...
				CakeCore.msgConsole("&4Error reading a line in skin data.");
			}
		}
	}
	...
	public CustomNPC createNPCUsernameSkin(Location location, String name, String skinUsername)
	{
		WorldServer s = ((CraftWorld) location.getWorld()).getHandle();
		World w = ((CraftWorld) location.getWorld()).getHandle(); //Warning: use NMS world
		TruenoNPCSkin npcSkin = new TruenoNPCSkin(CakeCore.instance, SkinType.IDENTIFIER, skinUsername);
