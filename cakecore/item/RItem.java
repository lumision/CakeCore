...
import cakecore.item.BonusStat.BonusStatType;
...
public class RItem
{
	...
	// BASE STATS
	public int levelRequirement, magicDamage, meleeDamage, cooldownReduction, critChance, critDamage, damageReduction, recoverySpeed;
	...
	//ENHANCEMENT
	private int tier;
	public int addedMagicDamage, addedMeleeDamage;
	...
	public int magicDamageAdd, meleeDamageAdd, damageReductionAdd, cooldownReductionAdd, recoverySpeedAdd, critChanceAdd, critDamageAdd;
	public float magicDamageMultiplier, meleeDamageMultiplier, attackSpeedMultiplier, xpMultiplierFood;
	...
	public Buff getBuff()
	{
		...
		if (bs == null)
			bs = Stats.createStats(buffName, itemVanilla).setAttackSpeedMultiplier(attackSpeedMultiplier).setMeleeDamageAdd(meleeDamageAdd)
			.setMeleeDamageMultiplier(meleeDamageMultiplier).setCooldownReductionAdd(cooldownReductionAdd).setCritChanceAdd(critChanceAdd)
	...
	public void setItemStats(ItemStack is)
	{
		...
		for (int i = 0; i < lore.size(); i++)
		{
			...
			} else if (line.startsWith("Expiry: ")) //Expiry: 15/10/2018, 9:38AM
			{
				try
				{
					Calendar calendar = Calendar.getInstance();
					String[] all = line.split(": ")[1].split(", ");
					...
					expiry = new TimeTrack(calendar.get(Calendar.YEAR), calendar.get(Calendar.DAY_OF_YEAR), 
							Integer.valueOf(time[0]) + (time[1].endsWith("PM") ? 12 : 0), 
							Integer.valueOf(time[1].substring(0, time[1].length() - 2)));
					loreRemove.add(i);
				} catch (Exception e) {}
			...
			} else if (line.startsWith("  Melee Damage: +"))
			{
				try
				{
					String[] numbers = line.split(": +")[1].split(" ");
					meleeDamage = Integer.parseInt(numbers[0]);
					if (numbers.length > 1)
					{
						addedMeleeDamage = Integer.parseInt(numbers[1].substring(2, numbers[1].length() - 1));
						meleeDamage -= addedMeleeDamage;
					}
					loreRemove.add(i);
				} catch (Exception e) {}
			...
			} else if (line.startsWith(" * Melee Damage: +"))
			{
				try
				{
					String num = line.split(": +")[1];
					if (line.endsWith("%"))
						meleeDamageMultiplier = CakeLibrary.convertAddedPercentageToMultiplier(
								Integer.parseInt(num.substring(0, num.length() - 1)));
					else
						meleeDamageAdd = Integer.parseInt(num);
	...
	public ItemStack createItem()
	{
		...
		if (!consumable)
		{
			...
			if (meleeDamage != 0)
				lore.add(CakeLibrary.recodeColorCodes(statColor + "  Melee Damage:" + statColorSecondary + " +" + (meleeDamage + addedMeleeDamage))
						+ (addedMeleeDamage > 0 ? " (+" + addedMeleeDamage + ")" : ""));
		...
		} else
		{
			...
			if (meleeDamageAdd != 0)
				lore.add(CakeLibrary.recodeColorCodes("&7 * Melee Damage: +" + meleeDamageAdd));
			if (meleeDamageMultiplier != 0)
				lore.add(CakeLibrary.recodeColorCodes("&7 * Melee Damage: +" + CakeLibrary.convertMultiplierToAddedPercentage(meleeDamageMultiplier) + "%"));
	...
	public static String getExpiryLine(TimeTrack expiry)
	{
		Calendar calendar = Calendar.getInstance();
		calendar.set(Calendar.DAY_OF_YEAR, expiry.dayOfYear);
		return CakeLibrary.recodeColorCodes("&7Expiry: " + calendar.get(Calendar.DAY_OF_MONTH) + "/" + 
		(calendar.get(Calendar.MONTH) + 1) + "/" + 
				expiry.year + ", " +
		(expiry.hour > 12 ? (expiry.hour - 12 + ":" + 
				(expiry.minute < 10 ? ("0" + expiry.minute) : expiry.minute) + "PM") : 
					(expiry.hour + ":" + 
				(expiry.minute < 10 ? ("0" + expiry.minute) : expiry.minute) + "AM")));
	}
	...
	public ItemStack createBaseItem()
	{
		...
		if (!consumable)
		{
			...
			if (meleeDamage != 0)
				lore.add(CakeLibrary.recodeColorCodes(statColor + "  Melee Damage:" + statColorSecondary + " +" + meleeDamage));
			if (damageReduction != 0)
				lore.add(CakeLibrary.recodeColorCodes(statColor + "  Damage Reduction:" + statColorSecondary + " +" + damageReduction + "%"));
			if (cooldownReduction != 0)
				lore.add(CakeLibrary.recodeColorCodes(statColor + "  Cooldown Reduction:" + statColorSecondary + " +" + cooldownReduction + "%"));
	...
	public void setTier(int num)
	{
		...
		addedMeleeDamage = (int) b - meleeDamage;
	}

	public void saveToFile(String fileName)
	{
		saveToFile(new File("plugins/CakeCore/items/" + fileName + ".yml"));
	}
	...
	public void saveToFile(File file)
	{
		...
		if (im != null)
		{
			...
			if (im instanceof BookMeta)
			{
				...
				{
					lines.add(" " + i + ":");
					for (String pageLine: pages.get(i).split("\\r?\\n"))
						lines.add("  " + pageLine);
				}
			}
		}
	...
	public static RItem readFromFile(File file)
	{
		try
		{
			...
			for (String line: lines)
			{
				...
				if (line.startsWith("  "))
				{
					while (bookPages.size() <= page)
						bookPages.add("");
					bookPages.set(page, bookPages.get(page) + line.substring(2) + "\n");
				}
				else if (line.startsWith(" "))
				{
					...
					{
						page = Integer.valueOf(line.substring(0, line.length() - 1));
					}
				} else
				{
					header = line;
			...
			CakeCore.msgConsole("&4Unable to read item file: " + file.getName());
			e.printStackTrace();
		}
		return null;
	}
