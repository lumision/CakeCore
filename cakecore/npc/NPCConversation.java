...
public class NPCConversation
{
	...
	public int checkCommands()
	{
		...
		} else if (part.string.toLowerCase().startsWith("@giveitem: "))
		{
			...
			RItem item = CakeCore.getItemFromDatabase(vars[1]);
			if (item == null)
			{
				CakeCore.msgConsole("&4Error while executing @giveitem in " + conversationData.npcName + "'s conversation data: &c" + vars[1] + " &4is not an existing RItem.");
				return -1;
			}
				...
				if (part.next.size() > 0 && part.next.get(0).type == ConversationPartType.PLAYER)
				{
					...
					for (int i = 0; i < part.next.size(); i++)
					{
						...
						if (n.string.startsWith("!itemdecision: "))
						{
							try
							{
								decision = CakeCore.getItemFromDatabase(n.string.split(": ")[1]).createItem();
							} catch (Exception e) 
							{
								CakeCore.msgConsole("Error while executing &4" + n.string + "&e, maybe the item does not exist?");
							}
						} else
						{
				...
				else if (part.next.size() > 0)
				{
					...
					try
					{
						while (next.string.startsWith("@"))
						{
							...
							{
								conversationUI.setItem(getPreviewSlot(), CakeCore.getItemFromDatabase(next.string.split(": ")[1]).createItem());
