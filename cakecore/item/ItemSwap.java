...
public class ItemSwap 
{
	...
	public static final File itemSwapsFile = new File("plugins/CakeCore/item-swaps.yml");
		...
		for (String line: CakeLibrary.readFile(itemSwapsFile))
		{
			try
			{
				...
				else if (line.startsWith(" "))
				{
					...
					{
						String[] split = line.split(", ");
						from = CakeCore.getItemFromDatabase(split[0]);
						to = CakeCore.getItemFromDatabase(split[1]);
			...
			{
				CakeCore.msgConsole("Error reading ItemSwap line: " + line);
				e.printStackTrace();
			}
			from = to = null;
		}
